...
deterministic_questions = ({"question": "Are you a parent or caregiver who has Autism concerns "
                                        "about your child?",
                            ...
                            "no": "This is not the site the user is looking for. Make references to"
                                  ...
                                  "looking "
                                  "for.\""}, {"question": "Has your child been screened for ASD?",
                                              "no": "Give an FAQ about screening. Inform the user "
                                                    ...
                                                    "MCHAT/POSI/SRS).",
                                              "yes": "Chat about the difference between screening "
                                                     "and evaluation."},
                           {"question": "Did the screening result indicate a full evaluation was "
                                        "needed?",
                            "no": "Chat about what if the user disagrees with "
                                  "the findings.",
...
with open("Milwaukee: Autism Evaluation and Therapy Services.md") as f:
    providers_prompt = {"role": "system", "content": "The following is information about service "
                                                     "providers nearby Milwaukee that you should "
                                                     "use in your responses.\n"
                                                     "" + f.read()}

with open("Patient Inputs and Outputs.md") as f:
    io_prompt = {"role": "system", "content": "Use this table to inform how you respond to "
                                              "prompts.\n" + f.read()}
...
def generate_history():
    ...
    for question in deterministic_questions:
        ...
        response = input("Assistant: " + question["question"] + "\nUser: ").lower() == "yes"
        if not response:
            history += no
            if "no" in question:
...
tokenizer = AutoTokenizer.from_pretrained("Open-Orca/Mistral-7B-OpenOrca", device_map="auto",
                                          offload_folder="offload",
                                          torch_dtype=torch.bfloat16)
generation_config = GenerationConfig(max_length=2000 * 30, temperature=1.1, top_p=0.95,
                                     repetition_penalty=1.0, do_sample=True, use_cache=True,
                                     eos_token_id=tokenizer.eos_token_id,
                                     pad_token_id=tokenizer.eos_token_id)

model = AutoModelForCausalLM.from_pretrained("Open-Orca/Mistral-7B-OpenOrca", device_map="auto",
                                             offload_folder="offload",
                                             torch_dtype=torch.bfloat16)
